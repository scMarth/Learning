...
void loop() {
   ...
   if (digitalRead(2) == HIGH && digitalRead(3) == HIGH && digitalRead(4) == LOW && digitalRead(5) == HIGH){
   ...
   } else if (digitalRead(2) == HIGH && digitalRead(3) == LOW && digitalRead(4) == HIGH && digitalRead(5) == HIGH){
      ...
      */
      ...
      // works way better than above, game reading keyboard input thinks it is pressing enter much more quickly
      Keyboard.press(0x20); // space (working in steam)
      delay(50);
      Keyboard.releaseAll();
      delay(100);
   } else if (digitalRead(2) == LOW && digitalRead(3) == HIGH && digitalRead(4) == HIGH && digitalRead(5) == HIGH){
      Keyboard.print("q");
   }
}