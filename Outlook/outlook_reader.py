...
class Outlook():
    ...
    def unreadString(self): # gets the last unread message as a string
        list = self.unreadIds()
        latest_id = list[-1]
        r, d = self.imap.fetch(latest_id, "(RFC822)")
        return d[0][1]

    def unread(self): # only the latest unread message (only 1 email)
        list = self.unreadIds()
        latest_id = list[-1]
...
def rm_cr_from_string(input_str): # remove carriage returns from string
    return_str = ""
    for char in [x for x in input_str if x not in [u'\x0d']]:
        return_str = return_str + str(char)
    return return_str

mail = Outlook()
mail.login('evilpolitician@hotmail.com', 'password')
mail.inbox()

with open(r"./output.txt", "w") as file:
    file.write(rm_cr_from_string(mail.unreadString()))
